...
(function () {
  ...
  UomoElements.Search = (function() {
    ...
    Search.prototype = Object.assign({}, Search.prototype, {
      ...
      //   const url = '/search'; // <-- URL string langsung

      //   fetch(url, { method: method })
      //     .then(response => {
      //       if (response.ok) {
      //         return response.text(); // atau `.json()` tergantung yang kamu kirim
      //       } else {
      //         return Promise.reject(response);
      //       }
      //     })
      //     .then(data => {
      //       // Kalau response-nya HTML, kirim ke _updateSearchResult
      //       _this._updateSearchResult(data, $form);
      //     })
      //     .catch(err => {
      //       _this._handleAjaxSearchError(err.message || 'Search failed', $form);
      //     });
      // }, 180),
